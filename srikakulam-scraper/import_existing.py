...
SOURCE_DIR = r"C:\Users\VIGNESH\Downloads\vizag-scraper\vizag-scraper\output\raw\srikakulam_jewellery\google"
DEST_RAW_DIR = "output/raw/srikakulam_jewellery/google"
DEST_LOG_DIR = "output/logs"

def clean_query_from_filename(filename):
    # e.g., grid__jewelry_store__Balaga__Srikakulam.json -> Jewelry Store in Balaga Srikakulam
    # e.g., text__jewellery_shop__Srikakulam.json -> Jewellery Shop Srikakulam
    ...
    if basename.startswith("grid__"):
        ...
        if len(parts) >= 4:
            ...
            return f"{kw} in {area} Srikakulam"
    elif basename.startswith("text__"):
        parts = basename.split("__")
        if len(parts) >= 3:
            kw = parts[1].replace("_", " ").title()
            return f"{kw} Srikakulam"
...
def run_import():
    ...
    state_file = os.path.join(DEST_LOG_DIR, "srikakulam_jewellery_google_state.json")
    ...
    print(f"Found {len(json_files)} existing Srikakulam raw files in Downloads.")
            ...
            with open(f, "r", encoding="utf-8") as file:
                ...
                for r in data:
                    ...
                    unique_places[place_id] = {
                        ...
                        "address": r.get("address", "Srikakulam, Andhra Pradesh"),
                        "phone": r.get("phone", ""),
                        "maps_url": r.get("maps_url") or f"https://www.google.com/maps/place/Srikakulam/data=!4m7!3m6!1s{place_id}",
                        "place_id": place_id
                    }
